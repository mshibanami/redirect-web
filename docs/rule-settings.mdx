...
This page provides detailed information of each setting in the *Edit Rule* screen in the RedirectWeb app.
...
* **Original** (Default)
    ...
    * ⚠️ This is slower than the *Declarative* type and may cause extra network requests.
...
**URL patterns** are used to match URLs in the *Redirect From*, *Excluded URL Patterns*, and *Capturing Group Processing* options. To specify them, you can choose either **Wildcard** or **Regular Expression**.
...
- To match `https://example.com/hello`, you can use `https://example.com/.*`. This will match any string after `https://example.com/`.
- To match `https://example.com/search?q=hello`, you can use `https://example.com/search\?q=(.*)`. This will match the value of the `q` parameter and store it in a capturing group. You can then reference it using `$1` in the *Redirect To* option.
- To match any URL that contains the word `blog`, you can use `.*blog.*`.

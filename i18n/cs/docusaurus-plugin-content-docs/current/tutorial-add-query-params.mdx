...
`$0` odkazuje na cílovou URL. Pokud se pokusíte přistoupit na `example.com/hello`, budete přesměrováni na `example.com/hello?layout=desktop`. Tato funkce se nazývá _substituce_.
...
- `(\(.*))?`: Shoduje se s řetězcem začínajícím `?`, což znamená parametry dotazu.
  - Toto se také shoduje s prázdným řetězcem pomocí kvantifikátoru `?` na konci vzoru, který _shoduje nula nebo jedenkrát_.
...
Toto není dokonalé řešení, protože přesměrovává `example.com/hello` na `example.com/hello?layout=desktop&`, což obsahuje zbytečné `&` na konci URL. Obecně to není velký problém, ale pokud si přejete ho odstranit, můžete použít _Zpracování zachycené skupiny_.

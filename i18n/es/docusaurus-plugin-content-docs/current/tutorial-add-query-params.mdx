...
> [!NOTE]
> RegExr muestra un error cuando no escapas `/` con `\`. Aunque puedes escaparlo, no es necesario ya que Redirect Web utiliza un motor diferente de Apple que no requiere escape.

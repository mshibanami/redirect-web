...
`$0` viitab siht-URL-ile. Kui proovite avada `example.com/hello`, suunatakse teid ümber `example.com/hello?layout=desktop`-le. Seda funktsiooni nimetatakse _asenduseks_.
...
- `("(.*))?`: Vastab stringile, mis algab `?`, mis tähendab päringuparameetreid.
  - See vastab ka tühjale stringile mustri lõpus oleva `?` kvantifikaatori abil, mis _vastab null või üks kord_.
  - Välimisele `()`-le ja sisemisele `()`-le saab hiljem viidata `$2`-ga ja `$3`-ga.

[RegExr](https://regexr.com) võib aidata teil üksikasju mõista.

> [!NOTE]
> RegExr näitab viga, kui te ei escape'i `/` `\`-ga. Kuigi saate seda escape'ida, ei ole see nõutav, kuna RedirectWeb kasutab Apple'i teistsugust mootorit, mis ei nõua escape'imist.

See ei ole täiuslik lahendus, kuna see suunab `example.com/hello` ümber `example.com/hello?layout=desktop&`-le, mis sisaldab URL-i lõpus ebavajalikku `&`. See ei ole üldiselt suur probleem, kuid kui soovite seda eemaldada, saate kasutada _püüdmisrühma töötlemist_.

...
* **Előfordulások cseréje**: Ez a csoportban egy vagy több karaktert lecserél, amely egy **Céllal** egyezik, egy **Csereszövegre**. Például, ha a *Csoport* `hello`, a *Cél* `l` és a Csereszöveg `y`, akkor az eredmény `heyyo` lesz.
...
- A `https://example.com/hello` illesztéséhez használhatja a `https://example.com/.*` mintát. Ez minden olyan karakterláncra illeszkedik, amely a `https://example.com/` után következik.
- A `https://example.com/search?q=hello` illesztéséhez használhatja a `https://example.com/search\?q=(.*)` mintát. Ez illeszkedik a `q` paraméter értékére, és rögzíti azt egy befogócsoportban. Erre azután hivatkozhat a `$1` segítségével az *Átirányítás ide* opcióban.
- Bármely olyan URL illesztéséhez, amely tartalmazza a `blog` szót, használhatja a `.*blog.*` mintát.

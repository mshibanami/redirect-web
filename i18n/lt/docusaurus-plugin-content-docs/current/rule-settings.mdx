...
Šiame puslapyje pateikiama išsami informacija apie kiekvieną nustatymą „RedirectWeb“ programos ekrane _Redaguoti taisyklę_.
...
- **Originalus** (Numatytasis)
  ...
  - Tai yra lėčiau nei _Deklaratyvus_ tipas ir gali sukelti papildomų tinklo užklausų.
...
Parinktis **Grupės fiksavimo apdorojimas** leidžia nurodyti, kaip apdoroti užfiksuotas grupes, kurias galite pakeisti parinktyje _Nukreipti į_ su `$1`, `$2`...
...
- **Pakeisti pasikartojimus**: Tai pakeičia vieną ar daugiau simbolių grupėje, atitinkančioje **Tikslą**, su **Pakeitimu**. Pavyzdžiui, jei _Grupė_ yra `hello`, o _Tikslas_ yra `l`, o Pakeitimas yra `y`, jis modifikuojamas į `heyyo`.
...
### Papildomi nustatymai {/* #advanced-options */}

- **[DNR] Pagrindinio rėmelio nukreipimas**: Ši parinktis pasiekiama tik DNR tipui. Ji leidžia nurodyti, kaip tvarkyti nukreipimą `main_frame` išteklių tipui. Galimos parinktys:
  - **Tiesioginis** (Raw): Nukreipia pagrindinį rėmelį į nurodytą URL be jokių pakeitimų.
  - **Per tarpinį puslapį**: Nukreipia pagrindinį rėmelį į tarpinį puslapį, kuris tada nukreipia į nurodytą paskirties URL. Tai naudinga atidarant programas naudojant giliąsias nuorodas, nes tarpinis puslapis gali apdoroti sudėtingesnę nukreipimo logiką.
...
**URL šablonai** naudojami URL atitikimui parinktyse _Nukreipti iš_, _Išskirti URL šablonai_ ir _Grupės fiksavimo apdorojimas_. Norėdami juos nurodyti, galite pasirinkti [Pakaitos simbolį](#url-pattern-wildcard) arba [Reguliariąją išraišką](#url-pattern-regular-expression).
...
- Norėdami atitikti `https://example.com/hello`, galite naudoti `https://example.com/.*`. Tai atitiks bet kokią eilutę po `https://example.com/`.
- Norėdami atitikti `https://example.com/search?q=hello`, galite naudoti `https://example.com/search\?q=(.*)`. Tai atitiks `q` parametro reikšmę ir išsaugos ją fiksavimo grupėje. Tada galite ją nurodyti naudodami `$1` parinktyje _Nukreipti į_.
- Norėdami atitikti bet kurį URL, kuriame yra žodis `blog`, galite naudoti `.*blog.*`.

Galite nurodyti fiksavimo grupes parinktyje _Nukreipti į_ arba _Pakeitimas_ naudodami `$1`, `$2`, ... arba `$0`, kad nurodytumėte visą atitikmenį.

...
- **Nevainojama maršrutēšana sistēmas migrācijas laikā:**
  Migrējot no mantotas iekšējās sistēmas (piemēram, `legacy-crm.internal.local`) į jaunu platformu (piemēram, `salesforce.com`), izplatiet `enforcedRuleSet`. Pat ja darbinieki izmanto vecās grāmatzīmes, viņi tiek automātiski pāradresēti uz jauno sistēmu, tādējādi novēršot IT atbalsta pieteikumus par novecojušām saitēm.
...
Video izmantotais plist fails ir pieejams šeit: **[sample-mdm.plist](/misc/sample-mdm.plist)**, kurā tiek izmantotas vārdnīcas (`<dict>`) atslēgas. Ir pieejams arī fails **[sample-mdm-json-string.plist](/misc/sample-mdm-json-string.plist)**, kurā tiek izmantotas JSON virknes atslēgas (tie paši dati).
...
> [!NOTE]
>
> **Vārdnīca (Dictionary) pret JSON virkni:**
> Lai gan vārdnīcas tips ir drošāks no tipu viedokļa, daži MDM risinājumi (piemēram, Omnissa Workspace ONE UEM) neatbalsta vārdnīcas tipu pārvaldītajā lietotņu konfigurācijā. Šādos gadījumos ieteicams izmantot JSON virknes tipu kā alternatīvu.
...
| ----------------------------- | ------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `enforcedRuleSet`             | [Noteikumu kopa](#rule-set-object)    | Noteikumu kopa, kas tiek piemērota ar augstāku prioritāti nekā parastie noteikumi. Saraksta pirmajam noteikumam ir prioritāte. Lietotāji nevar rediģēt, dzēst vai mainīt šo noteikumu secību. Tie vienmēr ir aktīvi, ignorējot lietotnes globālo atslēgšanas slēdzi, un netiek sinhronizēti caur iCloud.                           |
| `enforcedRuleSetJSONString`   | [JSON virkne](#json-string)           | Tas pats, kas `enforcedRuleSet`, taču noteikumu kopa tiek nodrošināta kā neapstrādāta JSON virkne. Tas ir noderīgi MDM platformām, kuras neatbalsta vārdnīcas tipu pārvaldītajā lietotņu konfigurācijā.                                                                                                                           |
| `prefilledRuleSet`            | [Noteikumu kopa](#rule-set-object)    | Noteikumu kopa, kas tiek izveidota noklusējuma piemēra noteikuma vietā, kad lietotājs pirmo reizi palaiž lietotni bez esošiem noteikumiem. Pēc izplatīšanas tie darbojas kā parastie noteikumi, ļaujot lietotājam tos modificēt, dzēst un sinhronizēt ar iCloud.                                                                   |
| `prefilledRuleSetJSONString`  | [JSON virkne](#json-string)           | Tas pats, kas `prefilledRuleSet`, taču noteikumu kopa tiek nodrošināta kā neapstrādāta JSON virkne. Tas ir noderīgi MDM platformām, kuras neatbalsta vārdnīcas tipu pārvaldītajā lietotņu konfigurācijā.                                                                                                                         |
...
| `fallback`                    | [Rezerves variants](#fallback-object) | Norādiet šo, ja konfigurācijas pārsniedz MDM datu apjoma ierobežojumus. Lietotne ielādēs konfigurāciju no URL, kas norādīts `fallbackDataURL`.                                                                                                                                                                                     |
| `fallbackJSONString`          | [JSON virkne](#json-string)           | Tas pats, kas `fallback`, taču rezerves konfigurācija tiek nodrošināta kā neapstrādāta JSON virkne. Tas ir noderīgi MDM platformām, kuras neatbalsta vārdnīcas tipu pārvaldītajā lietotņu konfigurācijā.                                                                                                                           |

Šīs atslēgas ir neobligātas.

### Konfigurācijas vērtību tipi {/* #configuration-value-types */}

#### Noteikumu kopa {/* #rule-set-object */}
...
#### Rezerves variants {/* #fallback-object */}

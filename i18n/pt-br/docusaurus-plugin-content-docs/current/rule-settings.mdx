...
Especifique a opção **Tipo** para controlar como o aplicativo lida com o redirecionamento. Você pode escolher entre estas:

* **Original** (Padrão)
    * Isso usa APIs Web tradicionais para controlar o redirecionamento. Além disso, usa [a API Tabs](https://developer.mozilla.org/en-US/docs/Mozilla/Add-ons/WebExtensions/API/tabs) para um fallback.
...
* **DNR** (Experimental para Safari):
    ...
    * ⚠️ Você não pode usar algumas opções, como [Processamento de Grupo de Captura](#capturing-group-processing) e [Padrões de URL Excluídos](#excluded-url-patterns), pois ainda não são suportadas pela API DNR.
...
A opção **Redirecionar De** permite que você especifique um padrão de URL de páginas da web das quais você deseja redirecionar. Você pode escolher [Curinga](#wildcard) ou [Expressão Regular](#regular-expression).
...
A opção **Navegadores Alvo** permite que você especifique a qual navegador a regra deve ou não deve ser aplicada.
...
* A caixa de seleção **Todos, exceto os selecionados** permite que você segmente todos os navegadores, exceto os que você escolheu.
...
* **Codificar/Decodificar URL**: Isso aplica [codificação percentual](https://en.wikipedia.org/wiki/Percent-encoding) ou decodificação a um grupo de captura. Por exemplo, se você codificar `https://example.com/hello`, ele é convertido para `https%3A%2F%2Fexample.com%2Fhello`. A decodificação funciona da maneira oposta.
...
You pode especificar padrões de URL excluídos usando tipos de padrão de Expressão Regular ou Curinga.

### Exemplos {#examples}

A opção **Exemplos** permite que você teste sua regra de redirecionamento fornecendo URLs de amostra. Ao adicionar uma URL de amostra, você pode verificar se a regra funciona como esperado antes de realmente aplicá-la.
...
**Padrões de URL** são usados para corresponder URLs nas opções *Redirecionar De*, *Padrões de URL Excluídos* e *Processamento de Grupo de Captura*. Para especificá-los, você pode escolher **Curinga** ou **Expressão Regular**.

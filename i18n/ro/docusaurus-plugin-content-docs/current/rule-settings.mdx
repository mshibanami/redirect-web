...
*   **Original** (Implicit)
    *   Aceasta utilizează API-uri Web tradiționale pentru a controla redirecționarea. Suplimentar, folosește [API-ul Tabs](https://developer.mozilla.org/en-US/docs/Mozilla/Add-ons/WebExtensions/API/tabs) ca soluție de rezervă.
    ...
    *   Puteți utiliza toate opțiunile, cu excepția [Tipurilor de Resurse](#resource-types) și a [Metodelor de Cerere](#request-methods).
...
*   **DNR** (Experimental pentru Safari):
    ...
    *   Aceasta vă permite să specificați [Tipuri de Resurse](#resource-types) și [Metode de Cerere](#request-methods).
    *   ⚠️ Nu puteți utiliza unele opțiuni, cum ar fi [Procesarea Grupurilor de Captură](#capturing-group-processing) și [Modelele URL Excluse](#excluded-url-patterns), deoarece acestea nu sunt încă acceptate de API-ul DNR.
    *   ⚠️ În Safari, în prezent nu puteți include bare verticale (`|`) în modelul dvs. de Expresie Regulară. [Detalii](https://github.com/mshibanami/redirect-web/issues/44)
...
Opțiunea **Redirecționare de la** vă permite să specificați un model URL al paginilor web de la care doriți să redirecționați. Puteți alege fie [Wildcard](#wildcard), fie [Expresie Regulară](#regular-expression).
...
Opțiunea **Tipuri de Resurse** vă permite să specificați categoriile de cereri web, cum ar fi imagini, scripturi JavaScript și foi de stil, la care se aplică regula.
...
> [!ATENȚIE]
> În Safari, această opțiune s-ar putea să nu fie disponibilă, deoarece implementarea Apple pentru tipul DNR nu pare să o gestioneze în prezent. Am trimis feedback către Apple ca FB14502272.
...
În plus, puteți modifica textele `$1`, `$2`, ... înainte de a efectua substituții. Verificați secțiunea [Procesarea Grupurilor de Captură](#capturing-group-processing) pentru detalii.
...
> [!ATENȚIE]
> Puteți deschide numai o aplicație care suportă [App Sandbox](https://developer.apple.com/documentation/security/app_sandbox). De asemenea, asigurați-vă că aplicația suportă deschiderea URL-ului pe care doriți să îl deschideți.
...
*   **Codificare/Decodificare URL**: Aceasta aplică [codificarea în procente](https://en.wikipedia.org/wiki/Percent-encoding) sau decodificarea unui grup de captură. De exemplu, dacă codificați `https://example.com/hello`, este convertit în `https%3A%2F%2Fexample.com%2Fhello`. Decodificarea funcționează invers.
...
> [!NOTĂ]
> Aceasta este pentru procesarea fiecărui grup de captură și nu afectează URL-urile care sunt excluse de [Modelele URL Excluse](#excluded-url-patterns). De exemplu, să presupunem că aveți această regulă:
>
...
> [!Notă]
> Deși substituția este o caracteristică a Expresiei Regulate, o puteți utiliza și pentru Wildcard, deoarece Redirect Web convertește intern Wildcard în Expresie Regulară.

### Expresie Regulară {#url-pattern-regular-expression}

**Expresia Regulară** (Regex) este un instrument puternic pentru potrivirea modelelor în text, propulsat de motorul de expresii regulate al Apple, descris [aici](https://developer.apple.com/documentation/foundation/nsregularexpression#1661042). Vă permite să definiți un model specific care se potrivește cu un set de șiruri. Iată câteva exemple.

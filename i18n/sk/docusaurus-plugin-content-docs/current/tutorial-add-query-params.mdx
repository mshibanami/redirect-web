...
`$0` odkazuje na cieľovú URL. Ak sa pokúsite pristupovať k `example.com/hello`, budete presmerovaní na `example.com/hello?layout=desktop`. Táto funkcia sa nazýva _substitúcia_.
...
- `(\(.*))?`: Zhoduje sa s reťazcom začínajúcim `?`, čo znamená parametre dotazu.
  - Toto sa tiež zhoduje s prázdnym reťazcom pomocou kvantifikátora `?` na konci vzoru, ktorý _zhoduje nula alebo jedenkrát_.
...
Toto nie je dokonalé riešenie, pretože presmeruje `example.com/hello` na `example.com/hello?layout=desktop&`, čo obsahuje zbytočné `&` na konci URL. Vo všeobecnosti to nie je veľký problém, ale ak ho chcete odstrániť, môžete použiť _Spracovanie zachytávacej skupiny_.

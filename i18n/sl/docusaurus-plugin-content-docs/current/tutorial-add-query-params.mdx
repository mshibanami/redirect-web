...
`$0` se nanaša na ciljni URL. Če poskusite dostopiti do `example.com/hello`, boste preusmerjeni na `example.com/hello?layout=desktop`. Ta funkcija se imenuje _nadomestitev_.
...
- `(\(.*))?`: Ujema se z nizom, ki se začne z `?`, kar pomeni parametre poizvedbe.
  - To se ujema tudi s praznim nizom z `?` kvantifikatorjem na koncu vzorca, ki _se ujema ničkrat ali enkrat_.
...
To ni popolna rešitev, saj preusmeri `example.com/hello` na `example.com/hello?layout=desktop&`, kar vključuje nepotreben `&` na koncu URL-ja. To ni velika težava na splošno, če pa ga želite odstraniti, lahko uporabite _Obdelavo zajetih skupin_.

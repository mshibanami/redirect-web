...
Denna sida ger detaljerad information om varje inställning på skärmen Redigera regel i Redirect Web-appen.
...
* **DNR** (Experimentell för Safari):
    ...
    * ⚠️ Du kan inte använda vissa alternativ, som [Bearbetning av fångstgrupper](#capturing-group-processing) och [Exkluderade URL-mönster](#excluded-url-patterns) eftersom de ännu inte stöds av DNR API.
...
![Typer som stöds: DNR](https://img.shields.io/badge/Types-DNR-blue)
...
Till exempel, om du ställer in `script`, kan du omdirigera JavaScript-filer som laddas av webbsidor.
...
![Plattformar som stöds: macOS](https://img.shields.io/badge/Platforms-macOS-white)
...
* Det finns 2 typer av webbläsare du kan välja:
    ...
    * **Användarspecifika webbläsare**: Genom att trycka på knappen Lägg till i popup-fönstret kan du välja vilken webbläsare som helst förutom Safari eller Safari Technology Preview som har Redirect Web-tillägget installerat.
...
> [!TIP]
> Du kan ange ett anpassat URL-schema för att öppna en app. Dessa är exempel på appar som stöder djup länkning:
>
...
![Typer som stöds: Original](https://img.shields.io/badge/Types-Original-blue) ![Plattformar som stöds: macOS](https://img.shields.io/badge/Platforms-macOS-white)
...
![Typer som stöds: Original](https://img.shields.io/badge/Types-Original-blue)
...
Dessa är hur man skapar fångstgrupper:
...
* **Ersätt förekomster**: Detta ersätter ett eller flera tecken i en grupp, matchade av ett **Mål**, med en **Ersättning**. Till exempel, om *Gruppen* är `hello` och *Målet* är `l` och Ersättningen är `y`, ändras det till `heyyo`.
...
* **Ren text** (Standard): Enkel text utan någon formatering.
* **Markdown**: Du kan använda [GitHubs Markdown-syntaxer](https://docs.github.com/en/get-started/writing-on-github/getting-started-with-writing-and-formatting-on-github/basic-writing-and-formatting-syntax), inklusive vissa tillägg som GFM inte stöder. Till exempel kan du skapa rubriker, listor, länkar och mer.
...
- För att matcha `https://example.com/hello`, kan du använda `https://example.com/.*`. Detta kommer att matcha vilken sträng som helst efter `https://example.com/`.
- För att matcha `https://example.com/search?q=hello`, kan du använda `https://example.com/search\?q=(.*)`. Detta kommer att matcha värdet för parametern `q` och lagra det i en fångstgrupp. Du kan sedan referera till det med `$1` i alternativet *Omdirigera till*.
- För att matcha vilken URL som helst som innehåller ordet `blog`, kan du använda `.*blog.*`.
...
Redirect Web drivs av [Apples Regex-motor](https://developer.apple.com/documentation/foundation/nsregularexpression#1661042).

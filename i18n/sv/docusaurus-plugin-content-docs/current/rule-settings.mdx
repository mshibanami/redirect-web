...
*   **DNR** (Experimentell för Safari):
    ...
    *   ⚠️ Du kan inte använda vissa alternativ, såsom [Bearbetning av fångstgrupper](#capturing-group-processing) och [Exkluderade URL-mönster](#excluded-url-patterns) eftersom de ännu inte har stödts av DNR API.
...
Alternativet **Resurstyper** låter dig ange kategorierna av webbförfrågningar, såsom bilder, JavaScript och stilmallar, som regeln gäller för.
...
> [!WARNING]
> I Safari kanske detta alternativ inte är tillgängligt eftersom Apples implementering för DNR-typen inte verkar hantera det för närvarande. Vi skickade feedback till Apple som FB14502272.
...
*   Det finns 2 typer av webbläsare du kan välja:
    *   **Webbläsare du kan välja som standard**: Du kan välja Safari, Chrome, Firefox, Edge, Opera, Arc, Brave och Vivaldi som standard. Detta skiljer inte varianter av samma webbläsare, såsom Safari och Safari Technology Preview.
    *   **Användarspecifika webbläsare**: Genom att trycka på knappen Lägg till i popup-fönstret kan du välja vilken webbläsare som helst utom Safari eller Safari Technology Preview som har RedirectWeb-tillägget installerat.
*   Kryssrutan **Alla utom valda** låter dig rikta in dig på alla webbläsare utom de du har valt.

> [!WARNING]
> RedirectWeb kan inte skilja om webbläsaren är **Safari** eller **Safari Technology Preview** eftersom appen kontrollerar tilläggets överordnade process för att upptäcka det, men dessa två webbläsare delar samma överordnade process (`/sbin/launchd`).
...
Dessutom kan du ändra texterna för `$1`, `$2`, ... innan du gör ersättningar. Kontrollera avsnittet [Bearbetning av fångstgrupper](#capturing-group-processing) för detaljer.
...
Om du vill ange en app du vill öppna destinations-URL:en med, använd kombinationsrutan **Applikation**. Detta är endast tillgängligt på macOS.
...
### Bearbetning av fångstgrupper {#capturing-group-processing}

![Supported Types: Original](https://img.shields.io/badge/Types-Original-blue)

Alternativet **Bearbetning av fångstgrupper** låter dig ange hur du ska bearbeta de fångade grupperna du kan ersätta i alternativet *Omdirigera till* med `$1`, `$2`...
...
> [!NOTE]
> Detta är för bearbetning av varje fångstgrupp och det påverkar inte vilka URL:er som exkluderas av [Exkluderade URL-mönster](#excluded-url-patterns). Låt oss till exempel säga att du har denna regel:
>
...
Alternativet **Exkluderade URL-mönster** låter dig ange de URL:er som inte omdirigeras. Detta kan vara användbart för att undvika omdirigeringsslingor eller för att exkludera vissa delar av en webbplats från att omdirigeras.
...
Alternativet **Kommentarer** låter dig lägga till anteckningar eller kommentarer om din omdirigeringsregel. Detta kan vara användbart för att hålla reda på varför du skapade en specifik regel eller för att ge sammanhang för andra som kan se dina regler.

Du kan skriva kommentarer i följande format:

*   **Vanlig text** (Standard): Enkel text utan någon formatering.
*   **Markdown**: Du kan använda [GitHubs Markdown-syntaxer](https://docs.github.com/en/get-started/writing-on-github/getting-started-with-writing-and-formatting-on-github/basic-writing-and-formatting-syntax), inklusive vissa tillägg som GFM inte stöder. Du kan till exempel skapa rubriker, listor, länkar och mer.
*   **AsciiDoc**: Du kan använda [AsciiDoc-syntaxer](https://asciidoc.org/) för att formatera dina kommentarer.

## URL-mönster {#url-pattern}

**URL-mönster** används för att matcha URL:er i alternativen *Omdirigera från*, *Exkluderade URL-mönster* och *Bearbetning av fångstgrupper*. För att ange dem kan du välja antingen **Wildcard** eller **Reguljärt uttryck**.
...
*   För att matcha `https://example.com/search?q=hello` kan du använda `https://example.com/search?q=*`. Detta matchar vilket värde som helst för parametern `q`.
...
> [!NOTE]
> Även om substitution är en funktion i reguljära uttryck, kan du också använda den för Wildcard eftersom RedirectWeb internt konverterar Wildcard till reguljära uttryck.

### Reguljärt uttryck {#url-pattern-regular-expression}

**Reguljärt uttryck** (Regex) är ett kraftfullt verktyg för att matcha mönster i text och används också flitigt inom programmering. Det låter dig definiera ett specifikt mönster som matchar en uppsättning strängar. Här är några exempel.

-   För att matcha `https://example.com/hello` kan du använda `https://example.com/.*`. Detta matchar vilken sträng som helst efter `https://example.com/`.
-   För att matcha `https://example.com/search?q=hello` kan du använda `https://example.com/search\?q=(.*)`. Detta matchar värdet för parametern `q` och lagrar det i en fångstgrupp. Du kan sedan referera till det med `$1` i alternativet *Omdirigera till*.
-   För att matcha vilken URL som helst som innehåller ordet `blog`, kan du använda `.*blog.*`.
...
Mer information om Regex-syntax kan hittas i resurser som [RegExr](https://regexr.com/).

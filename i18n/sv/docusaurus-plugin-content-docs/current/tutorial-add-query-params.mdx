...
`$0` refererar till mål-URL:en. Om du försöker komma åt `example.com/hello`, omdirigeras du till `example.com/hello?layout=desktop`. Denna funktion kallas _substitution_.
...
- `(\(.*))?`: Matchar en sträng som börjar med `?`, vilket betyder frågeparametrar.
  - Detta matchar också tom sträng med `?`-kvantifieraren i slutet av mönstret, vilket _matchar noll eller en gång_.
...
Detta är ingen perfekt lösning, eftersom den omdirigerar `example.com/hello` till `example.com/hello?layout=desktop&`, vilket inkluderar ett onödigt `&` i slutet av URL:en. Det är ingen stor sak i allmänhet, men om du vill ta bort det kan du använda _Bearbetning av fångstgrupper_.

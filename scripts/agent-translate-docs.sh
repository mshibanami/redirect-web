...
if [ -z "$sourceFiles" ]; then
  echo "Enter source files to translate (space-separated, or press Enter for all):"
  read -r input
  if [ -n "$input" ]; then
    sourceFiles="$input"
  fi
...
function check_and_filter_files() {
  ...
  for src in "${src_files[@]}"; do
    [ -e "$src" ] || continue
    local target="i18n/$lang/docusaurus-plugin-content-docs/current/$(basename "$src")"

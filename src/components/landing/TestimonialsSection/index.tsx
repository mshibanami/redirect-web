...
const testimonials: Testimonial[] = [
  ...
  // },
  {
    ...
    review: translate({
      ...
      message:
        'Nice, very nice. Does the job of 10 other extensions with more flexibility. Very future proof. Sleek UI, good price. Must buy for anyone into privacy redirects.',
      description: 'Testimonial review by a user',
    }),
  ...
  },
  {
    ...
    review: translate({
      ...
      message:
        'I needed an app to help curb my internet addiction. Every time my muscle memory takes me to YouTube, Hacker News or Twitter, I’ve set RedirectWeb to send me to a site filled with stoic quotes. That’s enough of a kick in the behind to stop me wanting to mindlessly browse.',
      description: 'Testimonial review by a user',
    }),
  ...
  },
  {
    ...
    review: translate({
      ...
      message:
        'Best app for redirecting links and websites to the destination of your choice. And more efficient than the others — it even redirects links blocked by your content blocker or firewall.',
      description: 'Testimonial review by a user',
    }),
...
function TestimonialCard({ testimonial, currentLocale }: TestimonialCardProps): ReactNode {
  ...
  const hasOriginal =
    testimonial.originalReview &&
    testimonial.originalLanguage &&
    testimonial.originalLanguage !== currentLocale;

  const displayReview =
    showOriginal && hasOriginal ? testimonial.originalReview : testimonial.review;
  const shouldTruncate = displayReview.length > maxReviewTextLength;
  const truncatedReview =
    shouldTruncate && !isExpanded ? displayReview.slice(0, maxReviewTextLength) : displayReview;
              ...
              {!showOriginal && (
                <span className={styles.translatedFrom}>
                  <Translate
                    ...
                    values={{
                      language: getLanguageName(testimonial.originalLanguage, currentLocale),
                    }}
                  >
    ...
    <section className={styles.testimonials}>
      ...
      <div className="row">
        {testimonials.map((testimonial, idx) => (
          <TestimonialCard key={idx} testimonial={testimonial} currentLocale={currentLocale} />
        ))}
      </div>
    </section>
  );
